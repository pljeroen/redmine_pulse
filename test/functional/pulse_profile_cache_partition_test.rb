...
# ============================================================================
# Scoring-profile cache partitioning (a security-critical guarantee).
#
# MIRRORS test/functional/cache_visibility_isolation_test.rb: the prior cross-serve guard
# was the visibility_context_id component; the NEW guard is the active_profile_id component.
# This suite proves the cache CANNOT serve profile A's snapshot for a profile-B request,
# and that the DEFAULT profile leaves the fingerprint byte-identical to the earlier
# nine-component hash (so a default-only install sees zero cache invalidation on upgrade).
#
# THE MAKE-OR-BREAK ASSERTIONS:
#   (1) distinct fingerprints for two different-weight profiles, and both differ from the
#       profile-omitted reference.
...
#
# THE ZERO-UPGRADE GUARD:
#   FP(x, "default") == FP(x, omitted)   byte-identical.
#   This guards (1) zero cache invalidation on upgrade for default-only installs, and
#   (2) that the impl OMITS the component for "default" (does NOT hash the literal "default").
#
# The suite needs the profile-aware API — SnapshotFingerprint's active_profile_id: kwarg,
# RedmineProfileProvider, and the Engine's profile_provider: dependency +
# project_projection(..., requested_id). The orchestrator runs it via
# scripts/test-redmine/run-tests.sh (NOT the pure `ruby -Itest -Ilib` lane, which cannot
# resolve Issue / Setting / the adapter constants).
#
# This suite runs on PostgreSQL (Issue.visible SQL composition on the deployed engine);
# skip (not fail) on non-Postgres so MySQL CI legs stay green.
class PulseProfileCachePartitionTest < ActiveSupport::TestCase
  ...
  # The REFERENCE fingerprint: the profile component OMITTED. Captured by resolving to the
  # reserved "default" id (which OMITS the component). We ALSO assert this equals the
  # nil/omitted path, so the reference is the nine-component vector, NOT a hash of the
  ...
  # ── default-fingerprint identity ────────────────────
  # The default profile OMITS the component => byte-identical to the profile-omitted hash.
  def test_default_fingerprint_is_byte_identical_to_omitted_component
    ...
    assert_equal fp_omitted, fp_default,
                 'FP(x,"default") must equal FP(x, omitted) byte-for-byte — the ' \
  ...
  def test_default_does_not_hash_the_literal_default_string
    ...
    # (the profile-omitted vector). A non-default id MUST differ from it (see cross-serve test).
    fp_default = fingerprint_for('default')
    fp_a = fingerprint_for('A')
    refute_equal fp_default, fp_a,
                 'a non-default profile id must fold a distinct extra component (fp_A != fp_default)'
    assert_equal fp_pre_reference, fp_default,
                 'the default fingerprint equals the profile-omitted reference (component omitted)'
  end

  # ── assertion (1): distinct fingerprints ────
  def test_distinct_profiles_yield_distinct_fingerprints
    fp_a = fingerprint_for('A')
    fp_b = fingerprint_for('B')
    fp9 = fp_pre_reference
    refute_equal fp_a, fp_b, 'fp_A != fp_B (distinct non-default profiles)'
    refute_equal fp_a, fp9, 'fp_A != the profile-omitted fingerprint (non-default folds an extra component)'
    refute_equal fp_b, fp9, 'fp_B != the profile-omitted fingerprint'
  end

  # ── assertion (2): two DISTINCT cached rows ────────────────────────
  def test_two_profiles_produce_two_distinct_cache_rows
    ...
    assert_equal after_a + 1, after_b,
                 'warming under a SECOND profile (B) must create a SECOND, ' \
    ...
    assert_equal rows.pluck(:snapshot_fingerprint).uniq.size, rows.count,
                 'each cached row has a distinct snapshot_fingerprint'
  end

  # ── assertion (3): NO CROSS-SERVE + (4) scores differ ──────────────
  ...
  def test_no_cross_serve_between_profiles_and_scores_differ
    ...
    assert_operator cache_row_count, :>, rows_after_a,
                    'a B-request after only-A-cached must MISS and warm a new ' \
    ...
    refute_equal proj_a.health.health_score, proj_b.health.health_score,
                 'scores under A and B must differ (profile weights applied). ' \
    ...
    assert_equal proj_a.health.health_score, proj_a2.health.health_score,
                 're-requesting under A returns A\'s own cached result, not B\'s'
    refute_equal proj_b.health.health_score, proj_a2.health.health_score,
                 'A\'s served score must not equal B\'s (no reverse cross-serve)'
  end

  # ── a profile-DEFINITION edit invalidates via settings_version_hash ──
  ...
  def test_profile_definition_edit_moves_the_fingerprint
    ...
    refute_equal sv_before, sv_after,
                 'a profile-DEFINITION edit under settings[\'pulse_profiles\'] must ' \
                 'move settings_version_hash (component 7)'
    refute_equal fp_before, fp_after,
                 'the moved settings_version_hash must move the fingerprint => ' \
                 'cache recompute (a profile-definition edit invalidates the cache)'
  end

  # ── the fingerprint is a PURE hash of inputs (no internal resolution) ─
  def test_snapshot_fingerprint_takes_profile_id_as_a_pure_kwarg
    ...
    assert_includes params, :active_profile_id,
                    'SnapshotFingerprint#initialize must take active_profile_id: as a kwarg'
    refute_includes params, :profile_provider,
                    'the fingerprint must NOT resolve profiles internally (no provider dep)'
    ...
    refute_match(/ProfileProvider|\.resolve\b/, src,
                 'SnapshotFingerprint must not reference ProfileProvider/#resolve')
  end
end
