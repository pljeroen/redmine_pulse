...
# IT-C4-07 (presentation slice) — FR-C4-08 (SHOULD) / FC-C4-17.
...
# The view reads ONLY the presenter output (COND-A4-001) — no direct AR/domain access.
#
# Falsifiable checks (FC-C4-17):
...
#
# HARNESS lane (RED-by-construction): HtmlPresenter references plugin constants resolved by
# Zeitwerk (I18n / MainConcernLabels / SignalResult), so it loads under the Redmine harness.
# RED until A9 adds active_profile_name + the switcher options to PanelView. The presenter
# fake Projection below carries the C4 fields (active_profile / active_profile_name /
# profile_options) that the pre-A9 PanelView does not surface — so panel_view raises
# NoMethodError / the assertions fail. Orchestrator runs it via run-tests.sh.
class PulseProfilePresentationTest < ActiveSupport::TestCase
  ...
  # === FC-C4-17 (a): active_profile_name is surfaced ============================
  def test_panel_view_exposes_active_profile_name
    ...
    assert_respond_to view, :active_profile_name,
                      'FC-C4-17(a): the PanelView must expose active_profile_name'
    assert_equal 'Risk Heavy', view.active_profile_name,
                 'FC-C4-17(a): active_profile_name == the active profile\'s name'
  end

  # === FC-C4-17 (b): the switcher lists one option per published profile ========
  def test_panel_view_switcher_lists_published_profiles
    ...
    assert_respond_to view, :profile_options,
                      'FC-C4-17(b): the PanelView must expose the switcher options (profile_options)'
    options = view.profile_options
    assert_equal PUBLISHED.size, options.size,
                 'FC-C4-17(b): one switcher option per published profile'
    ...
    assert_equal PUBLISHED.map(&:id).sort, values.sort,
                 'FC-C4-17(b): each option value == profile.id'
    assert_includes labels, 'Risk Heavy',
                    'FC-C4-17(b): each option carries the profile display name'
  end

  # === FC-C4-17 (b): the active profile is flagged selected in the switcher ======
  def test_switcher_flags_the_active_option_selected
    ...
    assert is_selected,
           'FC-C4-17: the active profile\'s switcher option must be flagged selected'
  end

  # === FC-C4-17 (c): the view-model exposes precomputed presentation state only ===
  # The switcher options are plain precomputed values (id + label + selected), NOT domain
  # ScoringProfile objects — so the ERB accesses no AR/domain object directly (COND-A4-001).
  def test_switcher_options_are_precomputed_presentation_values_not_domain_objects
    ...
    view.profile_options.each do |o|
      refute o.is_a?(Pulse::Domain::ScoringProfile),
             'FC-C4-17(c): switcher options must be precomputed view values, not domain ' \
