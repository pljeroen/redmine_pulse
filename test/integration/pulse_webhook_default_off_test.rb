...
# Security-critical off-by-default falsifier PLUS the additive-alongside-email wiring,
# exercised through the real scan_and_alert composition root on the Redmine harness.
#
# off-by-default: with EMPTY webhook endpoint settings (the shipped state), a scan
...
# additive wiring: with a configured+enabled mocked endpoint, the SAME transition scan
#   dispatches a webhook ALONGSIDE the email path; the email path is invoked unchanged and
#   advance_state runs even when the webhook dead-letters.
#
# Net::HTTP is intercepted at the INSTANCE-construction seam (Net::HTTP.new) — the
...
#
# Postgres-gated for parity with the sibling alert integration suites. Requires the
# dispatcher wired into ScanAndAlert.run and the OFF-by-default settings keys.
class PulseWebhookDefaultOffTest < ActiveSupport::TestCase
  ...
  # A recording Net::HTTP double mirroring the instance seam the transport now uses:
  ...
  # ── OFF BY DEFAULT — events fire, ZERO outbound HTTP ──────────────
  def test_events_fire_but_zero_outbound_http_when_no_endpoints_configured
    ...
    net_calls = []
    # A minimal interceptor: any Net::HTTP construction (the transport seam) is a
    ...
    assert_empty net_calls, 'ZERO outbound HTTP with the empty (shipped) endpoint config'
    # advance_state still ran.
    assert_equal 'amber', alert_store.find_by_project(@project.id)[:last_rag]
  end

  # ── additive — webhook dispatch alongside email on a transition ───
  def test_webhook_dispatched_alongside_email_on_transition
    ...
    refute_empty posts, 'a webhook POST occurred alongside email (additive wiring)'
    assert_operator ActionMailer::Base.deliveries.size, :>=, 1,
                    'the email path is still invoked unchanged (email delivered to the watcher)'
    assert_equal 'amber', alert_store.find_by_project(@project.id)[:last_rag],
                 'advance_state runs regardless of webhook outcome (best-effort)'
  end
  ...
  def test_advance_state_runs_even_when_webhook_dead_letters
    ...
    assert_equal 'amber', alert_store.find_by_project(@project.id)[:last_rag],
                 'state advances even when the webhook dead-letters (best-effort)'
  end
end
