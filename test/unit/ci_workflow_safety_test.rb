...
#
# It STATICALLY enforces the safe-CI security properties, the compatibility
# matrix, the host-neutral CI-script shape, the README matrix + CI badge, the
# CHANGELOG entry, and the committed-harness expectation. It reads the
# deliverable files and asserts; nothing is mocked, nothing is skipped.
#
# Where a target file is ABSENT the test FAILS (flunk) so the obligation stays
# live, not silently skipped — the same idiom as the static source-guard suites.
# Each assertion must FAIL if the file is absent, and FAIL if an unsafe
# workflow/script ships. FAIL CLOSED is the rule throughout.
# ─────────────────────────────────────────────────────────────────────────────
class CiWorkflowSafetyTest < Minitest::Test
  ...
  # Read raw file bytes, or FAIL if the deliverable is absent.
  ...
  def workflow_yaml
    ...
    flunk "#{WORKFLOW}: workflow YAML is not parseable: #{e.message}"
  end

  # ─────────── triggers — maintainer-only, never fork-triggerable ───────
  ...
  def test_ci01_triggers_are_maintainer_only_never_fork_triggerable
    ...
    refute_match(/pull_request_target/, src,
                 ...
                 'documented fork-PR secret-exfiltration exploit vector')
    ...
    refute_nil on_block,
               "#{WORKFLOW}: an `on:` trigger block must be present"
    ...
    triggers =
      ...
      else flunk("#{WORKFLOW}: unrecognized `on:` shape #{on_block.inspect}")
      end

    assert_includes triggers, 'push',
                    "#{WORKFLOW}: push MUST be a trigger so CI validates every commit"
    ...
    assert_empty fork_triggerable,
                 ...
                 "Actions. FAIL CLOSED. offending=#{fork_triggerable.inspect}"
  end

  # ───────────────────── zero secrets in the workflow ──────────────────
  ...
  def test_ci02_workflow_references_zero_secrets
    ...
    refute_match(/secrets\./, src,
                 ...
                 'key fork-PR safety property')
    refute_match(/\$\{\{\s*secrets/, src,
                 "#{WORKFLOW}: no `${{ secrets ... }}` expression permitted anywhere")
  end
  ...
  def test_ci03_top_level_permissions_contents_read_no_write_scope
    ...
    refute_nil perms,
               "#{WORKFLOW}: a TOP-LEVEL `permissions:` block must be declared"
    assert_kind_of Hash, perms,
                   "#{WORKFLOW}: top-level permissions must be a mapping with contents: read"
    assert_equal 'read', perms['contents'].to_s,
                 "#{WORKFLOW}: top-level permissions MUST grant `contents: read` " \
                 "(got #{perms['contents'].inspect})"
    ...
    assert_empty write_grants,
                 ...
                 "offending lines: #{write_grants.inspect}"
  end

  # ───────────────────── all actions pinned to a 40-hex SHA ────────────
  ...
  def test_ci04_every_uses_action_pinned_to_full_commit_sha
    ...
    refute_empty uses_lines,
                 ...
                 'so the SHA-pin guard has something to verify'
    ...
    uses_lines.each do |line|
      ...
      assert_match(/@[0-9a-f]{40}\z/, ref_part,
                   ...
                   "not a floating tag/branch — supply-chain pinning. offending: #{line.strip.inspect}")
    end
  end

  # ───────────────────── matrix — Ruby {3.2,3.4} × DB {pg,mysql} ───────
  ...
  def test_ci05_matrix_covers_ruby_3_2_and_3_4_and_postgres_and_mysql_on_redmine_6_1
    wf = workflow_yaml
    jobs = wf['jobs']
    refute_nil jobs, "#{WORKFLOW}: a `jobs:` block must be present"
    ...
    refute_empty matrices,
                 "#{WORKFLOW}: a job `strategy.matrix:` block must be present — the compat matrix"
    ...
    %w[3.2 3.4].each do |rb|
      assert_includes matrix_tokens, rb,
                      "#{WORKFLOW}: CI matrix MUST include Ruby #{rb}. got=#{matrix_tokens.inspect}"
    end
    assert(matrix_tokens.any? { |t| t.include?('postgres') || t == 'pg' },
           "#{WORKFLOW}: CI matrix MUST include a PostgreSQL DB leg. got=#{matrix_tokens.inspect}")
  ...
  def test_ci08_run_sh_exists_is_executable_and_bash_n_clean
    p = path(CI_SCRIPT)
    flunk "expected file missing: #{CI_SCRIPT}" unless File.exist?(p)
    assert File.executable?(p),
           "#{CI_SCRIPT}: the CI script MUST be executable (chmod +x)"

    # `bash -n` is a built-in syntax check (no install). Shell out to it.
    out = `bash -n #{p.shellescape} 2>&1`
    assert_equal 0, $?.exitstatus,
                 "#{CI_SCRIPT}: must pass `bash -n` syntax check. output: #{out}"
  end
  ...
  def test_a10_001_run_sh_standalone_lane_runs_full_unit_suite_not_only_domain
    ...
    assert(recursive_glob || explicitly_named,
           ...
           "three guards alongside the domain glob (coverage parity). " \
    ...
    # of the script. (Redundant with the assertion above, but pins the failure
    # reason to the specific `test/unit/domain/*`-only pattern.)
    ...
    refute(domain_only && !recursive_glob && !explicitly_named,
           ...
           "they run nowhere in CI. Broaden to test/unit/**/*_test.rb " \
           "or name the guards explicitly.")
  end

  # ───────────────────── immutable Redmine checkout ref ────────────

  # REPRODUCIBILITY.
  #
  # The label pins Redmine 6.1.2, but the checkout clones the MOVING
  ...
  def test_a10_002_workflow_checks_out_immutable_redmine_ref_not_moving_branch
    ...
    redmine_clone_lines.each do |line|
      ...
      refute_match(/-b\s+6\.1-stable\b/, line,
                   ...
                   "immutable v6.1.2 tag or a 40-hex commit SHA. offending: " \
                   "#{line.inspect}")
  ...
  def test_ci11_readme_has_compatibility_matrix_and_ci_badge
    ...
    assert_match(/MySQL/i, src, "#{README}: compatibility matrix must mention MySQL")

    # A Markdown table (a header separator row of pipes-and-dashes).
    assert_match(/\|[\s:-]*-{2,}[\s:|-]*\|/, src,
                 "#{README}: the compatibility matrix must be a Markdown table")
    ...
    assert(has_actions_badge || has_shields_ci_badge,
           ...
           'badge) linking to the project Actions run')
  end

  # ───────────────────── CHANGELOG entry references CI ─────────────────

  def test_ci12_changelog_has_ci_entry
    src = read_or_flunk(CHANGELOG)
    assert_match(/\bCI\b|continuous integration/i, src,
                 "#{CHANGELOG}: must have an entry referencing CI / continuous integration")
  ...
  def test_osi_ci01_harness_present_and_referenced
    assert File.exist?(path(HARNESS_SCRIPT)),
           "#{HARNESS_SCRIPT}: the local harness script must be present on disk"
    ...
    assert referenced,
           ...
           'scripts/ci/run.sh so local-dev parity is discoverable'
  end
