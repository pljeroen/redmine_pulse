...
# FR-C3-03, FR-C3-05 / FC-C3-07, FC-C3-08, FC-C3-09, FC-C3-10.
...
#
# RED NOW: Pulse::Domain::LensRanking is unimplemented -> require raises LoadError. GREEN
# after A9 creates lib/pulse/domain/lens_ranking.rb to the exact public API these tests pin.
class LensRankingTest < Minitest::Test
  ...
  # fields must be non-nil for an active result (FC-32); values are irrelevant to ranking
  ...
  # An INACTIVE SignalResult: all numeric fields nil (FC-32).
  ...
  # retain an always-active signal so the C1 RC-07 guard is satisfied). Used only for its
  ...
  # === FC-C3-08 / AC-C3-03: WORKED NUMERIC EXAMPLE (both active, both enabled) ===
  ...
  # === FC-C3-09 / AC-C3-04: all chosen signals inactive => nil (unrankable) =====
  ...
  # === FC-C3-09: all chosen signals disabled => nil (unrankable) ================
  ...
  # === FC-C3-10 / AC-C3-04: partial-inactivity excludes from BOTH sums ==========
  ...
  # === FC-C3-07 / FR-C3-05: a DISABLED chosen signal is excluded from BOTH sums ==
  ...
  # === FC-C3-07: purity — LensRanking does not mutate its inputs ================
  ...
  # === FC-C3-14 (structural): no DSL / eval surface in lens_ranking.rb ==========
  def test_no_eval_or_expression_surface_in_source
    ...
    %w[eval instance_eval class_eval module_eval binding].each do |tok|
      refute_match(/\b#{tok}\b/, src, "lens_ranking.rb must contain no #{tok} (no-DSL, FC-C3-14)")
    end
  end
end
