...
# Behavior-preservation guard for the ScoringConfig-adopts-WeightSet refactor.
...
#
# It does NOT duplicate the existing scoring_config_registry_test.rb:39 assertion (that
# file is UNCHANGED); it pins the additional #weights-is-a-plain-frozen-Hash + fail-loud
# guarantees that the WeightSet adoption must not weaken. The byte-identical golden gates
...
class ScoringConfigWeightsetAdoptionTest < Minitest::Test
  ...
  # === #weights is a PLAIN Hash (not a WeightSet) =====================
  def test_weights_is_a_plain_hash
    assert_instance_of Hash, CFG.new.weights,
                       'ScoringConfig#weights must return a plain Hash, not a WeightSet'
  end
  ...
  # === byte-identical Float values at each key (store-as-is) ==========
  ...
  # === #weights is a FROZEN Hash; [] access + values.sum unchanged =====
  ...
  # === fail-loud ArgumentErrors PRESERVED after the refactor ==========
