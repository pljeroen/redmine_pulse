...
# I18n sweep — STATIC source guard (no Redmine env) for the badge JS i18n wiring.
#
...
#
# Until the JS is wired to the blob labels, ragWord() hardcodes the four words and
# buildChip() builds `'Health: ' + word` and the JS never references data.labels; this
# guard PASSES once the JS consumes the server-provided labels.
class I18nBadgeJsLabelsGuardTest < Minitest::Test
  ...
  # ── the JS reads localized RAG words from the blob labels object ──
  ...
  def test_js_reads_labels_object_from_blob
    ...
    assert_match(/\blabels\b/, src,
                 'project_list_badge.js MUST consume a `labels` object from the ' \
                 'inline blob (it currently holds its own hardcoded vocabulary)')
    assert_match(/(?:data|portfolio|blob)\s*(?:\.|\[['"])labels|\.labels\b/, src,
                 'the JS MUST read `labels` off the parsed blob (e.g. data.labels) ' \
  ...
  def test_js_does_not_hardcode_rag_words_as_chip_text
    ...
    %w[Green Amber Red].each do |word|
      refute_match(/return\s+['"]#{word}['"]/, src,
                   "the badge JS MUST NOT `return '#{word}'` as hardcoded RAG " \
                   'chip text — the localized word comes from the server blob labels')
    end
    refute_match(/return\s+['"]No data['"]/, src,
                 'the badge JS MUST NOT `return \'No data\'` as hardcoded chip text ' \
                 '(it comes from label_pulse_rag_no_data via the blob)')
  end

  # ── the aria label is built from a server format string, not a JS-side prefix ──
  ...
  def test_js_does_not_hardcode_health_aria_prefix
    ...
    refute_match(/['"]Health:\s*['"]\s*\+/, src,
                 "the badge JS MUST NOT build the aria label as the hardcoded " \
                 "'Health: ' + word prefix — use the server-provided aria format string")
  end
  ...
  def test_js_retains_english_fallback_when_labels_absent
    src = js
    # The pure renderer must remain fail-safe: if the blob carries no labels, it falls
    # back to an English default rather than rendering an empty/undefined chip. After the
    # fix the JS reads labels WITH a `||`/`&&` fallback on the SAME line. We require the
    # fallback construct to co-occur with a `labels` reference (so the pre-existing
    # `default:` switch case does NOT satisfy it).
    fallback_with_labels = src.lines.any? do |line|
      line.include?('labels') && (line =~ /\|\||&&/)
    end
    assert fallback_with_labels,
           'the JS MUST read the blob labels WITH an English fallback on the ' \
