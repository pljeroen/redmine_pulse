...
# Webhook STATIC source guards (path-based; no Redmine env). Covers the mechanical
# layer-placement gates for the outbound-webhook code:
#   * lib/pulse/domain/ stays HTTP/DNS/socket-clean (the domain uses only the stdlib).
#   * Net::HTTP ONLY in http_webhook_dispatcher.rb; Resolv ONLY in ssrf_guard.rb;
#     OpenSSL::HMAC adapter-only.
#   * the WebhookDispatcher PORT names no adapter constant and no Net::HTTP.
#   * the request cycle (app/controllers + app/helpers) references the webhook
#     adapters ONLY in the explicit test-event action of pulse_controller.rb.
#   * the endpoint secret is not interpolated into any Rails.logger call.
#   * every new webhook .rb carries a GPL-2.0 header.
#
# Where a target file is ABSENT the test FAILS (flunk) so the obligation is LIVE, not
# silently skipped. Pure lane:
...
class WebhookStaticGuardsTest < Minitest::Test
  ...
  # The HTTP/DNS/socket token set the domain must never contain.
  ...
  # ── the domain layer stays HTTP/DNS/socket-clean ─────────────────
  def test_domain_layer_has_no_outbound_http_or_dns_or_socket
    ...
    files.each do |f|
      ...
      refute_match(HTTP_TOKENS, src,
                   "#{File.basename(f)}: domain must stay HTTP/DNS/socket-clean")
      refute_match(/\bResolv\b/, src, "#{File.basename(f)}: no DNS in the domain")
      refute_match(/OpenSSL::HMAC/, src, "#{File.basename(f)}: no HMAC crypto in the domain")
    end
  end

  # ── Net::HTTP ONLY in the http dispatcher adapter ────────────────
  def test_net_http_confined_to_the_http_dispatcher_adapter
    ...
    Dir[path('lib/pulse/**/*.rb')].each do |f|
      ...
      refute_match(/Net::HTTP/, src,
                   "#{f.sub(ROOT + '/', '')}: Net::HTTP is confined to http_webhook_dispatcher.rb")
    end
  end

  # ── Resolv ONLY in the SSRF guard adapter ────────────────────────
  def test_resolv_confined_to_the_ssrf_guard_adapter
    ...
    Dir[path('lib/pulse/**/*.rb')].each do |f|
      ...
      refute_match(/\bResolv\b/, src,
                   "#{f.sub(ROOT + '/', '')}: Resolv is confined to ssrf_guard.rb")
    end
  end

  # ── the PORT names no adapter constant and no Net::HTTP ──────────
  def test_port_is_pure_no_adapter_no_http
    src = read_or_flunk(PORT)
    refute_match(HTTP_TOKENS, src, 'the WebhookDispatcher port must contain no outbound HTTP')
    refute_match(/HttpWebhookDispatcher|SsrfGuard|WebhookPayloadSerializer/, src,
                 'the port must not name any adapter constant')
    refute_match(/ActiveRecord|ApplicationRecord|\bSetting\b/, src,
                 'the port is stdlib + Pulse::Domain only')
  end

  # ── request cycle refs the webhook adapters ONLY in the test-event action ─
  def test_request_cycle_references_webhook_adapters_only_in_pulse_controller
    ...
    request_cycle.each do |f|
      ...
      refute_match(webhook_consts, src,
                   ...
                   "cycle outside the pulse_controller test-event action")
    end
  end

  # ── the endpoint secret is not interpolated into a log call ──
  def test_secret_not_interpolated_into_any_logger_call
    ...
    src.scan(/Rails\.logger\.\w+\([^\n]*\)/).each do |line|
      refute_match(/secret/i, line,
                   "the endpoint secret must NEVER be interpolated into a log line: #{line}")
    end
    # Also: no bare `#{...secret...}` interpolation anywhere in a string that is logged.
    refute_match(/logger\.\w+\([^)]*#\{[^}]*secret/i, src,
                 'no secret interpolation into any logger call')
  end

  # ── GPL-2.0 header on every new webhook .rb ───────────────────────
  def test_new_webhook_files_carry_gpl_header
    present = NEW_WEBHOOK_RUBY.select { |r| File.exist?(path(r)) }
    flunk 'no new webhook .rb files found' if present.empty?
    present.each do |rel|
      head = File.read(path(rel))[0, 600]
      assert_match(/GPL|GNU General Public License/i, head,
                   "#{rel}: every new webhook .rb must carry a GPL-2.0 header")
    end
  end

  # ── init.rb declares BOTH webhook keys, defaulting OFF ──
  def test_init_declares_both_webhook_keys_default_off
    ...
    assert_match(/pulse_webhook_endpoints_global/, src,
                 'init.rb must declare pulse_webhook_endpoints_global')
    assert_match(/pulse_webhook_endpoints_per_project/, src,
                 'init.rb must declare pulse_webhook_endpoints_per_project')
  ...
  # ── no new DB migration / no new rake task introduced by the webhook feature ───────
  def test_c7_adds_no_new_migration_or_rake_task
    # The webhook feature is settings-only + additive rake wiring; endpoints live in plugin
    # Settings. (This is a coarse guard: no webhook migration or webhook rake task is added.)
    migrations = Dir[path('db/migrate/*webhook*')]
    assert_empty migrations, 'the webhook feature introduces NO webhook DB migration (endpoints live in Settings)'
    rake = File.exist?(path('lib/tasks/pulse.rake')) ? File.read(path('lib/tasks/pulse.rake')) : ''
    refute_match(/webhook/i, rake, 'the webhook feature adds NO new webhook rake task — it wires into scan_and_alert')
  end
end
